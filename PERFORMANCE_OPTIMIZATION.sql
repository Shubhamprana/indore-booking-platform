-- PERFORMANCE OPTIMIZATION FOR BOOKNOW DATABASE
-- This file contains optimizations to improve login and registration performance

-- 1. CREATE ESSENTIAL INDEXES FOR FAST LOOKUPS
-- User table indexes for auth and lookups
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_users_email_lower ON users (LOWER(email));
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_users_referral_code_lower ON users (LOWER(referral_code));
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_users_user_type ON users (user_type);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_users_created_at ON users (created_at);

-- User stats indexes for dashboard performance
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_stats_user_id ON user_stats (user_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_stats_last_calculated ON user_stats (last_calculated_at);

-- Referrals indexes for faster referral processing
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_referrals_referrer_status ON referrals (referrer_id, status);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_referrals_referred_user ON referrals (referred_user_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_referrals_created_at ON referrals (created_at);

-- Activities indexes for activity feeds
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_business_dashboards_user_id ON business_dashboards (user_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_business_dashboards_pro_expires ON business_dashboards (pro_expires_at);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_business_dashboards_plan_status ON business_dashboards (plan_status);

-- 2. OPTIMIZED FUNCTIONS FOR FASTER PERFORMANCE
...
-- 3. CREATE MATERIALIZED VIEW FOR FASTER DASHBOARD QUERIES
...
-- 4. OPTIMIZED BUSINESS FUNCTIONS
...
-- Performance notes:
-- 1. Use CONCURRENTLY for index creation to avoid blocking
-- 2. Materialized view should be refreshed periodically (every hour)
-- 3. Monitor slow queries with pg_stat_statements
-- 4. Consider partitioning user_activities table if it grows large
-- 5. Use connection pooling (pgbouncer) for better performance 