-- FIX BUSINESS FUNCTIONS V3 - Final Fixed Database Functions
-- Run this in Supabase SQL Editor to fix business registration errors

-- 1. First, ensure business_dashboards table has proper constraints
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE conname = 'business_dashboards_user_id_key'
    ) THEN
        -- Add unique constraint on user_id
        ALTER TABLE business_dashboards 
        ADD CONSTRAINT business_dashboards_user_id_key UNIQUE (user_id);
    END IF;
EXCEPTION
    WHEN others THEN
...
-- 2. Function to grant pro subscription to business users
...
BEGIN
    ...
    IF NOT dashboard_exists THEN
        -- Create new dashboard with pro subscription
...
-- 3. Function to grant initial business bonus (3 months pro) - SIMPLIFIED
CREATE OR REPLACE FUNCTION grant_initial_business_bonus(business_user_id UUID)
RETURNS void AS $$
DECLARE
    already_given BOOLEAN := false;
...
BEGIN
    ...
    -- Check if dashboard exists and if bonus already given
    SELECT EXISTS(
        SELECT 1 FROM business_dashboards WHERE user_id = business_user_id
    ) INTO dashboard_exists;
    
    IF dashboard_exists THEN
        SELECT COALESCE(initial_pro_months_given, false) INTO already_given
        FROM business_dashboards 
        WHERE user_id = business_user_id;
    END IF;
    ...
    IF NOT already_given THEN
        ...
        -- If dashboard was created by grant_pro_subscription, we're done
        -- If it existed before, update the flag
        IF dashboard_exists THEN
...
-- 4. Function to check if user is business (if not exists from performance optimization)
...
-- 5. Function to process business referrals
...
-- 6. DROP AND RECREATE business subscription status view to fix data type conflict
DROP VIEW IF EXISTS business_subscription_status CASCADE;

CREATE VIEW business_subscription_status AS
SELECT 
    u.id as user_id,
    u.full_name,
    u.business_name,
...
-- Success message
SELECT 'Business functions fixed successfully! No more ON CONFLICT errors! 🚀' as result; 